...
class WorkerProcess(multiprocessing_context.Process):
    ...
    def _hard_exit(self, e):
        ...
        try:
            display.debug(f"WORKER HARD EXIT: {to_text(e)}")
    ...
    def _run(self):
        ...
        try:
            ...
            display.debug(f"running TaskExecutor() for {self._host}/{self._task}")
            ...
            display.debug(
                f"done running TaskExecutor() for {self._host}/{self._task} [{self._task._uuid}]"
            )
            self._host.vars = {}
            self._host.groups = []

            # put the result on the result queue
            display.debug(f"sending task result for task {self._task._uuid}")
            ...
            display.debug(f"done sending task result for task {self._task._uuid}")
            ...
            if not isinstance(e, (IOError, EOFError, KeyboardInterrupt, SystemExit)) or isinstance(e, TemplateNotFound):
                try:
                    ...
                    display.debug(f"WORKER EXCEPTION: {to_text(e)}")
                    display.debug(f"WORKER TRACEBACK: {to_text(traceback.format_exc())}")
                finally:
                    self._clean_up()
