...
class GalaxyRole(object):
    ...
    def __init__(self, galaxy, api, name, src=None, version=None, scm=None, path=None):
        ...
        display.debug(f'Validate TLS certificates: {self._validate_certs}')
                ...
                for meta_main in self.META_MAIN:
                    ...
                    if os.path.isfile(meta_path):
                        try:
                            ...
                            display.vvvvv(f"Unable to load metadata for {self.name}")
                            return False
                        break
        ...
        if self._install_info is None:
            ...
            if os.path.isfile(info_path):
                try:
                    ...
                    display.vvvvv(f"Unable to load Galaxy install info for {self.name}")
                    return False
                finally:
                    f.close()
    ...
    def fetch(self, role_data):
        ...
        if role_data:
            ...
            if "github_user" in role_data and "github_repo" in role_data:
                archive_url = f'https://github.com/{role_data["github_user"]}/{role_data["github_repo"]}/archive/{self.version}.tar.gz'
            else:
                archive_url = self.src

            display.display(f"- downloading role from {archive_url}")

            try:
                url_file = open_url(archive_url, validate_certs=self._validate_certs, http_agent=user_agent())
                temp_file = tempfile.NamedTemporaryFile(delete=False)
                while data := url_file.read():
                    temp_file.write(data)
                temp_file.close()
                return temp_file.name
            except Exception as e:
                display.error(f"failed to download the file: {to_text(e)}")
    ...
    def install(self):
        ...
        if self.scm:
            ...
            if os.path.isfile(self.src):
                ...
                if not role_data:
                    raise AnsibleError(
                        f"- sorry, {self.src} was not found on {self.api.api_server}."
                    )
                ...
                if not self.version:
                    ...
                    if role_versions and to_text(self.version) not in [a.get('name', None) for a in role_versions]:
                        raise AnsibleError(
                            f"- the specified version ({self.version}) of {self.name} was not found in the list of available versions ({role_versions})."
                        )
        ...
        if tmp_file:
            ...
            display.debug(f"installing from {tmp_file}")
            ...
            for member in members:
                for meta_main in self.META_MAIN:
                    if meta_main in member.name:
                        ...
                        if (
                            meta_file
                            and len(meta_parent_dir) < len(archive_parent_dir)
                            or not meta_file
                        ):
            ...
            installed = False
            while not installed:
                display.display(f"- extracting {self.name} to {self.path}")
                try:
                    if os.path.exists(self.path):
                        if not os.path.isdir(self.path):
                            ...
                            raise AnsibleError(
                                f"the specified role {self.name} appears to already exist. Use --force to replace it."
                            )
                        else:
                    ...
                    if e.errno == errno.EACCES and len(self.paths) > 1:
                        ...
                        if len(self.paths) > current:
                            ...
                            error = False
                    if error:
                        raise AnsibleError(f"Could not update files in {self.path}: {to_native(e)}")

                # return the parsed yaml metadata
            display.display(f"- {str(self)} was installed successfully")
            if not self.src or not os.path.isfile(self.src):
                try:
                    os.unlink(tmp_file)
                except (OSError, IOError) as e:
                    display.warning(f"Unable to remove tmp file ({tmp_file}): {to_text(e)}")
            return True
            ...
            for meta_requirements in self.META_REQUIREMENTS:
                ...
                if os.path.isfile(meta_path):
                    try:
                        ...
                        display.vvvvv(f"Unable to load requirements for {self.name}")
                    finally:
                        f.close()
